...
export class FlowerRingSystem {
  ...
  getMaterialFromCache(texture) {
    ...
    let textureId = texture.uuid || "default";
    ...
    // 🔥 PENTING: biar warna tidak berubah
  ...
  cleanupMaterialCache() {
    ...
    if (this.materialCache.size > this.maxMaterialCacheSize) {
      ...
      const toRemove = entries.slice(
        0,
        this.materialCache.size - this.maxMaterialCacheSize,
      );
  ...
  createHardcodedFallbackTexture() {
    try {
      ...
      console.error(
        "❌ Critical error in createHardcodedFallbackTexture:",
        error,
      );
    }
  }
    ...
    try {
      ...
      try {
        ...
        ctx.arc(
          imageWidth / 2,
          imageHeight / 2,
          Math.min(imageWidth, imageHeight) / 3,
          0,
          Math.PI * 2,
        );
        ctx.fill();
      ...
      for (let y = 0; y < canvas.height; y++) {
        for (let x = 0; x < canvas.width; x++) {
          ...
          if (dist < radius) {
            const alpha = Math.pow(dist / radius, 0.5); // lebih tajam
  ...
  triggerFlyingEffect() {
    ...
    this.flowers.forEach((flower, index) => {
      ...
      flower.userData.targetScale = flower.scale
        .clone()
        .multiplyScalar(this.flyingConfig.scaleMultiplier);
  ...
  redistributeFlowers() {
    ...
    if (this.redistributeCount >= 1) {
      console.log(
        "🛑 Đã đạt giới hạn redistribute (1 lần), dừng để bảo vệ memory",
      );
      return;
    }
  ...
  animate(factor) {
    if (this.flowerRing) {
      if (!this.isFlying) {
        this.flowerRing.rotation.y += 0.002 * factor;
      } else {
        if (this.isPaused) {
        ...
        this.flowers.forEach((flower) => {
          if (!flower.userData.isFlying) {
            if (
              currentTime - flower.userData.startTime >
            ...
            flower.rotation.x +=
              Math.sin(currentTime * this.flyingConfig.rotationSpeed) * 0.0005;
            flower.rotation.y +=
              Math.cos(currentTime * this.flyingConfig.rotationSpeed) * 0.0005;
            flower.rotation.z +=
              Math.sin(currentTime * this.flyingConfig.rotationSpeed * 0.5) *
              0.0005;
          }
        });
      }
  ...
  preloadTextures(urls) {
    ...
    try {
            ...
            (texture) => {
              ...
              try {
                ...
                try {
                  ...
                  console.warn(
                    "⚠️ Error drawing image with orientation:",
                    drawError,
                  );
                  // Fallback: vẽ hình tròn đơn giản
                  ctx.fillStyle = "#ff69b4";
                ...
                // Đảm bảo texture có uuid
                if (!processedTexture.uuid) {
                  processedTexture.uuid =
                    "preloaded_" + Date.now() + "_" + index;
                }
      ...
      return Promise.all(loadPromises).then((textures) => {
        ...
        console.log(
          `✅ Successfully preloaded ${validTextures.length} textures`,
        );
