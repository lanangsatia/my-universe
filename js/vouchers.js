...
export function updateTotalPrice(getDynamicPrice) {
  ...
  if (selectedVoucher) {
    ...
    const discountAmount = Math.round(
      (price * selectedVoucher.discountValue) / 100,
    );
...
export async function loadUserVouchers(getDynamicPrice) {
  ...
  if (!uid) {
    voucherList.innerHTML =
      '<span style="color:#e53935;">Kamu perlu login untuk melihat voucher!</span>';
    return;
  }
  try {
    ...
    if (!data.data.length) {
      voucherList.innerHTML =
        '<span style="color:#888;">Bạn không có voucher nào cả!</span>';
      return;
    }
    vouchers = data.data;
    ...
    voucherList.innerHTML =
      '<span style=\"color:#e53935;\">Không thể tải voucher!</span>';
  }
}
  ...
  if (voucherList) {
    voucherList.addEventListener("change", (e) => {
      if (e.target.name === "voucher") {
        const checkboxes = voucherList.querySelectorAll(
          'input[name=\"voucher\"]',
        );
