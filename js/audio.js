...
export class AudioManager {
  constructor() {
    ...
    // music default
    this.defaultAudioUrl = "assets/musics/skyfullofstars.mp3";
  ...
  cleanupAudioCache() {
    ...
    if (this.audioCache.size > this.maxCacheSize) {
      ...
      const toRemove = entries.slice(
        0,
        this.audioCache.size - this.maxCacheSize,
      );
  ...
  getCurrentAudioInfo() {
    return {
      ...
      name: this.audioList[this.currentAudioIndex]
        ?.split("/")
        .pop()
        ?.replace(".mp3", ""),
